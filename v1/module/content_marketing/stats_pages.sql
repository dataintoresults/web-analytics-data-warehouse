...
-- Compute the read ratio on all the history (since start of tracking)
end_of_page_events as (
	select clean_page as page, sum(unique_events) as finish_count
	from clean_events
	where category = 'Scroll' and label = '75%'
		and date > '20200115'
	group by 1
),
...
read_ratio as (
	select page, coalesce(finish_count, 0)*1.0/start_count as read_ratio
	from start_of_page_events 
	left join end_of_page_events using (page)
),
-- Unique page view over the last 13 weeks (up to end of last week)
weeks_ref as (
	select to_char(ts, 'YYYY-MM') as year_week, row_number() over (order by ts) as rk
	from generate_series(current_date - interval'13 months', current_date - interval'1 months', '1 month') ts
),
pages_stats_row as (
	...
	where unique_pageviews > 0
		and to_char(to_date(date, 'YYYYMMDD'), 'YYYY-MM') < to_char(current_date, 'IYYY-MM')
		and to_char(to_date(date, 'YYYYMMDD'), 'YYYY-MM') >= to_char(current_date - interval'13 months', 'IYYY-MM')
	group by 1, 2	
),
-- SEO Data
clean_seo as (
	...
	where to_char(to_date(date, 'YYYY-MM-DD'), 'YYYY-MM') < to_char(current_date, 'YYYY-MM')
		and to_char(to_date(date, 'YYYY-MM-DD'), 'YYYY-MM') >= to_char(current_date - interval'13 months', 'YYYY-MM')
	group by 1, 2
),
-- Pivot the data
pages_stats as (
...
from pages_stats 
left join read_ratio using (page)
order by pageviews_month_13 desc